"""Independent Renderline control plane.

This service owns command delivery, durable billable ledgers, result/state
receipts, and selection relay. Hermes Desktop is only a replaceable client.
"""
...
from fastapi import APIRouter, FastAPI, HTTPException, Request, WebSocket, WebSocketDisconnect, status as http_status
...
def _control_token() -> str:
    ...
    try:
        ...
        return token if len(token) >= 43 else ""
    except OSError:
        return ""
...
app = FastAPI(title="Renderline Sidecar", version=SERVICE_VERSION, docs_url=None, redoc_url=None)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def health():
    return {
        "ok": True,
        "service": "renderline-sidecar",
        "version": SERVICE_VERSION,
        "schemaVersion": API_SCHEMA_VERSION,
        "commandClients": len(_COMMAND_CLIENTS),
    }
